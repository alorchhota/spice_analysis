...
args <- add_argument(args, '--cname',
                     help='comma-separted covariate names - empty string to remove all.',
                     default='')
...
cov_names = parse_delimitted_string(cov_names_input, delim = ',')
...
### use desired covariates only
if(length(cov_names) > 0){
  cov_df = cov_df[, intersect(colnames(cov_df), cov_names), drop=F]
}
